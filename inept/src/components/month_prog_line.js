...
import './month_prog_line.css';

function first_day(color){
    let content = [];
    content.push(<polygon key='first_day_pol' points="0,0 4,0 4,1 5,2 4,3 4,4 0,4" fill={color} className='DayBlocks'/>);
    content.push(<text key='first_day_text' x={2} y={2.1} fill="white" fontSize={2} textAnchor='middle' dominantBaseline="middle" fontWeight='Bold'>1</text>);
    return (content);
}
...
function middle_days(start_x, color){
    ...
    content.push(<polygon key={'mid_day_pol_'+day} points={pnts} fill={color} className='DayBlocks'></polygon>);
    content.push(<text key={'mid_day_text_'+day} x={start_x+2.4} y={2.1} fill="white" fontSize={2} textAnchor='middle' dominantBaseline="middle" fontWeight='Bold'>{day}</text>);
    return (content);
}
...
function last_day(start_x,color){
    ...
    content.push(<polygon key='last_day_pol' points={pnts} fill={color} className='DayBlocks'/>);
    content.push(<text key='last_day_text' x={start_x+2.4} y={2.1} fill="white" fontSize={2} textAnchor='middle' dominantBaseline="middle" fontWeight='Bold'>{day}</text>);
    return (content);
}
